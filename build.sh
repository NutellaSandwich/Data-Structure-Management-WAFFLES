...
function run () {
    ...
    echo
    echo "   [INFO]    Copying over source files into jar..."
    ...
    echo
    echo "[SUCCESS]    Copying done, new jar file:"
    echo "             $WORK_DIR/target/waffles-current-build-port-$PORTNO.jar"
    echo
    echo "   [INFO]    Running website..."

    java -Xmx1700m --add-opens=java.base/sun.net.www.protocol.jar=ALL-UNNAMED -XX:+IgnoreUnrecognizedVMOptions -jar "$WORK_DIR/target/waffles-current-build-port-$PORTNO.jar" --server.port=$PORTNO
